...
class _AgendaState extends State<Agenda> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Row(
            children: [
              const Text("Mostra fatti", style: TextStyle(fontSize: 12)),
              Switch(
                value: _showDone,
                onChanged: (val) {
                  setState(() {
          ...
          child: FutureBuilder<InfoReturn>(
            ...
            builder: (context, snapshotOuter) {
              if (snapshotOuter.connectionState == ConnectionState.waiting) {
                ...
                List<List<Info>?> sezioni = snapshotOuter.data!.info;

                Future<List<Info>?> compitiFuture = Future.value(sezioni.isNotEmpty ? sezioni[0] : null);
                Future<List<Info>?> notizieFuture = Future.value(sezioni.length > 1 ? sezioni[1] : null);
                Future<List<Info>?> verificheFuture = Future.value(sezioni.length > 2 ? sezioni[2] : null);
                Future<List<Info>?> altroFuture = Future.value(sezioni.length > 3 ? sezioni[3] : null);
                Future<List<Info>?> perDomaniFuture = Future.value(sezioni.length > 4 ? sezioni[4] : null);

                int timesD = 0, timesDD = 0, times3 = 0, times4_7 = 0, times8_15 = 0, times15 = 0;
  ...
  Widget displayTime(String title, int value){
    return Column(
      ...
      children: [
        Row(
          children: [
            const Text("       Eventi per", style: TextStyle(fontSize: 13)),
        ...
        FutureBuilder<List<Info>?>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              List<Info> allInfos = snapshot.data!;
              
              List<Info> loadedInfos = _showDone 
                  ? allInfos 
                  : allInfos.where((i) => !(_prefs?.getBool(i.storageKey) ?? false)).toList();

              Map<String, List<Info>> groupedByDate = {};
              for (var info in loadedInfos) {
                String dateKey = info.data.substring(0, 10);
                groupedByDate.putIfAbsent(dateKey, () => []).add(info);
              }
                            ...
                            child: Column(
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: Text(
                                    "${getData(dateKey)} - ${getDistanza(dateKey)} ${infosForDate.length == 1 ? "" : "(${infosForDate.length})"}",
                                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white),
                                  ),
                                ),
                                Row(
                                  children: infosForDate.map((info) => Padding(
  ...
  Color? getColor(String dataFine) {
    try {
      ...
      if (differenzaGen.inDays == 0) {
        return const Color.fromRGBO(255, 100,100, 0.5).withOpacity(opacita);
      }
  ...
  String getData(String dataString){
    List<String> nomiGiorni = ['Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato', 'Domenica'];
    List<String> nomiMesi = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembre'];
    DateTime data = DateTime.parse(dataString);
    return "${nomiGiorni[data.weekday - 1]} ${data.day} ${nomiMesi[data.month - 1]}";
  }

  String getDistanza(String dataString){
    DateTime today = DateTime.now();
    DateTime data = DateTime.parse(dataString);
    Duration differenza = data.difference(DateTime(today.year, today.month, today.day));
    if(differenza.inDays == 0) return "Oggi";
    if(differenza.inDays == 1) return "Domani";
    if(differenza.inDays == 2) return "Dopodomani";
    return "Tra ${differenza.inDays} giorni";
  }
}
